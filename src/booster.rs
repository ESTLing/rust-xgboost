...
impl Booster {
    ...
    ///
    /// `importance_type` is one of: `"weight"`, `"gain"`, `"cover"`, `"total_gain"`,
    /// `"total_cover"`. See the [XGBoost docs](https://xgboost.readthedocs.io/en/latest/python/python_api.html#xgboost.Booster.get_score)
    /// for descriptions.
    ///
    /// `feature_names` maps C-returned `f0`, `f1`, … indices to user-provided names
    /// (e.g. `&["age".into(), "income".into()]`). Out-of-range indices keep their raw
    /// `"fN"` string. Pass `&[]` to keep all raw names.
    ///
    /// Returns an ordered `Vec` sorted by descending importance score. Features with
    /// zero importance are not included.
    ///
    /// # Panics
    ///
    /// Panics on `gblinear` multi-class models — those return per-class vectors which
    /// this method does not support.
    pub fn get_score(&self, importance_type: &str, feature_names: &[String]) -> XGBResult<Vec<(String, f32)>> {
        let config = json_cstr!("importance_type" => importance_type);
        let mut n_features: xgboost_sys::bst_ulong = 0;
        let mut out_features: *mut *const raw::c_char = ptr::null_mut();
        let mut out_dim: xgboost_sys::bst_ulong = 0;
        let mut out_shape: *const xgboost_sys::bst_ulong = ptr::null();
        let mut out_scores: *const libc::c_float = ptr::null();

        xgb_call!(xgboost_sys::XGBoosterFeatureScore(
            self.handle,
        ...
        if out_dim != 1 {
            return Err(XGBError::new(format!(
                "get_score: expected scalar scores (out_dim=1), got out_dim={}. \
                 Multi-class gblinear models are not supported.",
                out_dim
            )));
        }

        let n = n_features as usize;
        if n == 0 || out_features.is_null() || out_scores.is_null() {
            return Ok(Vec::new());
        }
        let feature_ptrs = unsafe { slice::from_raw_parts(out_features, n) };
        let scores = unsafe { slice::from_raw_parts(out_scores, n) };

        let mut result: Vec<(String, f32)> = feature_ptrs
            .iter()
            .zip(scores)
            .filter_map(|(&ptr, &score)| {
                let raw = unsafe { ffi::CStr::from_ptr(ptr) }.to_str().ok()?;
                // Parse "fN" → index N, then map to user name
                let name = if let Some(idx_str) = raw.strip_prefix('f') {
                    if let Ok(idx) = idx_str.parse::<usize>() {
                        feature_names.get(idx).cloned().unwrap_or_else(|| raw.to_string())
                    } else {
                        raw.to_string()
                    }
                } else {
...
mod tests {
    ...
    fn get_feature_importance() {
        // Use enough data that trees actually split on features
        let n = 100;
        let n_features = 5;
        ...
        for i in 0..n {
            let x1 = i as f32 % 3.0;
            let x2 = (i as f32 * 1.7) % 5.0;
            data.extend_from_slice(&[x1, x2, (i % 7) as f32, i as f32 * 0.1, (i * i) as f32 % 10.0]);
            labels.push(if x1 + x2 > 3.0 { 1.0 } else { 0.0 });
        }
        let mut dtrain = DMatrix::from_dense(&data, n).unwrap();
        dtrain.set_label(&labels).unwrap();

        let mut bst = Booster::new(n_features).expect("Creating Booster failed");
        bst.set_params(&[("max_depth", "3"), ("eta", "0.3"), ("objective", "binary:logistic")])
            .expect("set_params failed");
        bst.train(&dtrain, 10, &[]).unwrap();

        let scores = bst.get_score("weight", &[]).unwrap();
        assert!(!scores.is_empty(), "Should have at least one feature with importance");
        assert!(scores.len() <= 5, "Cannot have more features than input dimensions");
        // Verify sorted descending
        for i in 1..scores.len() {
            assert!(scores[i - 1].1 >= scores[i].1, "Scores should be sorted descending");
        }
    }
